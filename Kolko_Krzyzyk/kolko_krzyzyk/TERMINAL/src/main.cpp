// Program do gry w "kolko i krzyzyk" z wykorzystaniem procedury mini-max.
// Jest to wersja "beta" i wymaga dopracowania (np. dolozenia pelnej kontroli bledow)
...
// ____________________________________________________________________
int WystarczajaGleboko(int p) { // Czy glebokosc MiniMax jest OK (stala GLEBOKOSC_OK jest zdefiniowana na poczatku pliku)
 if (p==GLEBOKOSC_OK)
	return 1;
 else
	return 0;
...
// ____________________________________________________________________
void Zeruj(KTO *plansza) // zeruje teren gry
{
for (int i=0;i<9;i++) plansza[i]=nic;
}
// ____________________________________________________________________
KANDYDACI Generuj(KTO plansza[9]) // generuje mozliwe kandydatury nastepnego ruchu
{
KANDYDACI t;
 t.cpt=-1;
 for(int i=0;i<9;i++)
 if (plansza[i]==nic)
	  t.tbl[++t.cpt]=i; // zapamietanie wolnej pozycji
return t;
}
// ____________________________________________________________________
...
if (a)
	return 1;    // robimy to aby wiedziec kto wygral
if (b)
	return 3;
if (c)
	return 6;
...
bool SprawdzCzyKoniecGry(KTO plansza[9],KTO gracz) {
 if (KoniecGry(plansza)) {
  ...
  if (gracz==czlowiek)
		...
		cout << "WYGRAL KOMPUTER! Nacisnij klawisz\n";
  cin.get();
  return true;
 }
...
int Ewaluacja(KTO plansza[9],KTO gracz) {
 ...
 if (result){
   if (plansza[result]==Nastepny_Gracz(gracz))
	 return minus_niesk;   			// pozycja wygrywajaca dla przeciwnika
   else
	return  plus_niesk;    			// pozycja wygrywajaca dla nas
  }
...
// ____________________________________________________________________
KTO NastepnyGracz(KTO gracz, int glebokosc) { // dla ktorego gracza wykonujemy ewaluacje mini-max
 if ((glebokosc % 2) == 0)
	return gracz;
 else
...
POLE MiniMax(KTO plansza[9],int glebokosc,KTO gracz) {
 POLE res;
 KANDYDACI nastepni;
 if (WystarczajaGleboko(glebokosc)) {// wezel uznawany za polozony dostatecznie gleboko
...
// ____________________________________________________________________
bool Zajete(KTO plansza[9],int ruch) {// Pole wolne?
 if (ruch==-1)
	return true;
 if (plansza[ruch] != nic)
	 return false;
...
// ____________________________________________________________________
void WybierzRuch(KTO gracz,KTO plansza[9],int &ruch) { // wybor ruchu zalezy od tego, kto gra
 if (gracz==czlowiek)
  do {
